...
import { Layers, Settings, Users, FileText, Sparkles, Inbox, UploadCloud, CheckCircle2, AlertTriangle, XCircle, Paperclip } from "lucide-react";
...
function RFxTab({ categories, configs, vendors, rfxList, setRfxList }) {
  ...
  async function handleImageDraft() {
    ...
    setChatLog((l) => [...l, { role: "user", text: `📎 ${img.name}` }, { role: "assistant", text: "…", thinking: true }]);
    setDrafting(true);
    setError("");
    try {
      const drafted = await draftLineItemsFromImage(current.columnSchema, img, current.lineItems);
      setCurrent((c) => ({ ...c, lineItems: drafted }));
      setChatLog((l) => [...l.filter((m) => !m.thinking), { role: "assistant", text: `Read ${img.name} and drafted ${drafted.length} line item(s). Review and edit below.` }]);
    } catch (e) {
        ...
        <>
          ...
          <div style={{ border: `1px solid ${C.line}`, borderRadius: 12, background: C.paperRaised, marginBottom: 24, overflow: "hidden", boxShadow: SHADOW_SM }}>
              ...
              <div style={{ padding: "12px 16px", maxHeight: 160, overflow: "auto", borderBottom: `1px solid ${C.line}` }}>
                {chatLog.map((m, i) => (
                  <div key={i} style={{ marginBottom: 8, fontFamily: FONT_UI, fontSize: 13 }}>
                    ...
                    {m.thinking ? (
                      <span style={{ color: C.slate, display: "inline-flex", verticalAlign: "middle" }}>
                        <ThinkingDots />
                      </span>
                    ) : (
            ...
            <div style={{ padding: 10, display: "flex", flexDirection: "column", gap: 6 }}>
              {attachedImage && (
                <div style={{ display: "inline-flex", alignItems: "center", gap: 6, alignSelf: "flex-start", background: C.amberSoft, borderRadius: 6, padding: "3px 8px", fontFamily: FONT_UI, fontSize: 11.5, color: C.amber }}>
                  <FileText size={11} />
                  <span style={{ maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{attachedImage.name}</span>
                  <button onClick={() => setAttachedImage(null)} style={{ background: "none", border: "none", color: C.amber, cursor: "pointer", fontSize: 12, lineHeight: 1, padding: 0 }}>×</button>
                </div>
              )}
              <div style={{ display: "flex", gap: 8 }}>
                <label style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 38, border: `1px solid ${C.line}`, borderRadius: 8, cursor: "pointer", color: C.slate, flexShrink: 0 }}>
                  <Paperclip size={15} />
                  <input type="file" accept="image/*" style={{ display: "none" }} onChange={(e) => e.target.files[0] && setAttachedImage(e.target.files[0])} />
                </label>
                <input
                  style={{ ...inputStyle, flex: 1 }}
                  placeholder={attachedImage ? "Optional note, or just hit Send to scan the image" : 'e.g. "30 line items, 3-ply and 5-ply boxes in 5 sizes, quote per 100 pieces"'}
                  value={chatInput}
                  onChange={(e) => setChatInput(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && (attachedImage ? handleImageDraft() : handleChatSubmit())}
                  disabled={drafting}
                />
                <button
                  onClick={attachedImage ? handleImageDraft : handleChatSubmit}
                  disabled={drafting || (!chatInput.trim() && !attachedImage)}
